...
known_data_summary = known_data %>%
  filter(use == "use") %>%
  group_by(class, drug, species, S_I_R) %>%
  summarise(`abs avg l2fc` = mean(abs(value), na.rm = T),
            `number of samples` = n_distinct(Strain)) %>%
  mutate(text = round(`abs avg l2fc`, digits = 1),
         drug = recode(drug,
         "Cefaz" = "Cefazolin",              
         "cz" = "Cefazolin",
         ...
         SIR = factor(substr(S_I_R, 1, 1), level = c("S", "I", "R")),
         dataset = "Melanie") %>%
  select(!c("S_I_R")) %>%
  rbind(output_data_summary %>% 
          dplyr::mutate(SIR = factor(substr(Susceptibility, 1, 1), level = c("S", "I", "R")),
                        dataset = "Clinical")
        ) %>%
  filter(drug != "Cefazolin")
...
known_data_summary %>%
  ...
  ggplot(aes(y = `number of samples`, x = SIR , fill = `abs avg l2fc`)) +
    ...
    facet_nested(species + dataset ~ class + drug, scales = "free_y", space = "free_y") +
    NULL
```

```{r, fig.height=12, fig.width=6}
known_data_summary %>%
  mutate(`sum abs avg l2fc` = `abs avg l2fc` * `number of samples`) %>%
  group_by(class, species, SIR, dataset) %>%
  summarise(`number of samples` = sum(`number of samples`, na.rm = T),
            `abs avg l2fc` = sum(`sum abs avg l2fc`, na.rm = T) / `number of samples`) %>%
  mutate(text = round(`abs avg l2fc`, digits = 1)) %>%
  ggplot(aes(y = `number of samples`, x = SIR , fill = `abs avg l2fc`)) +
    geom_col(color = "black") +
    scale_fill_viridis(option="inferno", begin = .15, end = 1, direction = -1) +
  ...
  scale_y_log10(breaks = scales::log_breaks(n = 10)) +
    ...
    facet_nested(species + dataset ~ class, scales = "free_y", space = "free_y") +
    NULL
...
graphDrug("Gent", plot = "SPDwSusceptibility")
```
...
SVMdata_onSPD = SVMdata_onSPD %>% 
 ...
 SVMdata_onSPD %>% 
   ...
   mutate(class = recode(class,
                        "Fq" = "Fluoroquinolone",
                        "Gent" = "Aminoglycoside")) %>%
   group_by(Species, class, drug) %>%
   calculate_performance() %>%
   write.csv(paste0(dir2, "/results_perdrug.csv"))
 ...
 SVMdata_onSPD %>% 
   ...
   group_by(class) %>%
   calculate_performance() %>%
   write.csv(paste0(dir2, "/results_perclass.csv"))
 ...
 SVMdata_onSPD %>% 
   ...
   mutate(class = recode(class,
                        ...
                        "Gent" = "Aminoglycoside")) %>%
   calculate_performance() %>%
   write.csv(paste0(dir2, "/results_all.csv"))
...
plot = CZ_MIC %>% 
  filter(NewMIC != "not measured") %>%
  mutate_at(c("Clinical SIR", "NewSIR"), ~factor(.x, levels = c("S", "I", "R"))) %>%
  group_by(`Clinical SIR`, NewSIR, pred_S_R, Species) %>%
  summarise(n = n_distinct(samples)) %>% #view()
  ggplot(aes(y = `Clinical SIR`, x = `NewSIR`, fill = pred_S_R, shape = Species)) +
  geom_rect(xmin = 0.5, xmax = 1.5, ymin = 0.5, ymax = 1.5, fill = "grey80", color = "black") +
  geom_rect(xmin = 1.5, xmax = 2.5, ymin = 1.5, ymax = 2.5, fill = "grey80", color = "black") +
  geom_rect(xmin = 2.5, xmax = 3.5, ymin = 2.5, ymax = 3.5, fill = "grey80", color = "black") +
  geom_beeswarm(aes(size = n), color = "black", cex = 7, method = "center") +
  scale_size(range = c(5, 15)) + 
  theme(panel.background = element_rect(fill = "white", color = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_line(size = 1, linetype = 'solid', color = "black")) +
  scale_color_manual(values = c("S" = "#0072B2", "R" = "#D55E00")) +
  scale_shape_manual(values = c("E. coli" = 21, "K. pneumoniae" = 24)) +
  #scale_fill_manual(values = c("Correct" = "#0072B2", "Correct, Predicted by KPC" = "#009E73", "Incorrect" = "#D55E00")) +
  NULL
